...
func (r *tenantRepository) GetByID(ctx context.Context, id uuid.UUID) (*entity.Tenant, error) {
	...
	query := `SELECT id, name, display_name, slug, is_active, created_at, updated_at, deleted_at FROM tenants WHERE id = $1 AND deleted_at IS NULL`
...
func (r *tenantRepository) GetBySlug(ctx context.Context, slug string) (*entity.Tenant, error) {
	var tenant entity.Tenant
	query := `SELECT id, name, display_name, slug, is_active, created_at, updated_at, deleted_at FROM tenants WHERE slug = $1 AND deleted_at IS NULL`
...
func (r *tenantRepository) List(ctx context.Context, offset, limit int) ([]*entity.Tenant, error) {
	...
	query := `SELECT id, name, display_name, slug, is_active, created_at, updated_at, deleted_at FROM tenants WHERE deleted_at IS NULL ORDER BY created_at DESC LIMIT $1 OFFSET $2`
