...
func (r *userRepository) GetByID(ctx context.Context, id uuid.UUID) (*entity.User, error) {
	...
	query := `SELECT id, email, username, password_hash, first_name, last_name, avatar_url, locale, is_active, is_blocked, block_reason, blocked_at, blocked_by, totp_secret, totp_enabled, totp_verified_at, created_at, updated_at, deleted_at FROM users WHERE id = $1 AND deleted_at IS NULL`
...
func (r *userRepository) GetByEmail(ctx context.Context, email string) (*entity.User, error) {
	var user entity.User
	query := `SELECT id, email, username, password_hash, first_name, last_name, avatar_url, locale, is_active, is_blocked, block_reason, blocked_at, blocked_by, totp_secret, totp_enabled, totp_verified_at, created_at, updated_at, deleted_at FROM users WHERE email = $1 AND deleted_at IS NULL`
...
func (r *userRepository) GetByUsername(ctx context.Context, username string) (*entity.User, error) {
	var user entity.User
	query := `SELECT id, email, username, password_hash, first_name, last_name, avatar_url, locale, is_active, is_blocked, block_reason, blocked_at, blocked_by, totp_secret, totp_enabled, totp_verified_at, created_at, updated_at, deleted_at FROM users WHERE username = $1 AND deleted_at IS NULL`
...
func (r *userRepository) List(ctx context.Context, offset, limit int) ([]*entity.User, error) {
	...
	query := `SELECT id, email, username, password_hash, first_name, last_name, avatar_url, locale, is_active, is_blocked, block_reason, blocked_at, blocked_by, totp_secret, totp_enabled, totp_verified_at, created_at, updated_at, deleted_at FROM users WHERE deleted_at IS NULL ORDER BY created_at DESC LIMIT $1 OFFSET $2`
