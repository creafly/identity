...
type TOTPHandler struct {
	...
	attemptTracker service.TOTPAttemptTracker
}

func NewTOTPHandler(totpService service.TOTPService, userService service.UserService, attemptTracker service.TOTPAttemptTracker) *TOTPHandler {
	return &TOTPHandler{
...
func (h *TOTPHandler) Enable(c *gin.Context) {
	...
	if h.attemptTracker.IsLocked(userIDStr) {
		c.JSON(http.StatusTooManyRequests, gin.H{"error": messages.TOTP.Locked})
		return
	}
	...
	if err != nil {
		...
		case service.ErrTOTPInvalidCode:
			h.attemptTracker.RecordFailedAttempt(userIDStr)
	...
	h.attemptTracker.ClearAttempts(userIDStr)
