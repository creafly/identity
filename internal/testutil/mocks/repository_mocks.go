...
func (m *OutboxRepositoryMock) GetPending(ctx context.Context, limit int, maxRetries int) ([]*outbox.Event, error) {
	...
	for _, event := range m.events {
		if event.Status == "pending" && event.RetryCount < maxRetries {
...
func (m *OutboxRepositoryMock) GetPendingForRetry(ctx context.Context, limit int, maxRetries int) ([]*outbox.Event, error) {
	...
	var result []*outbox.Event
	for _, event := range m.events {
		if event.Status == "pending" && event.RetryCount > 0 && event.RetryCount < maxRetries {
			if event.NextRetryAt != nil && event.NextRetryAt.Before(time.Now()) {
				result = append(result, event)
				if len(result) >= limit {
					break
...
func (m *OutboxRepositoryMock) AddEvent(event *outbox.Event) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.events[event.ID] = event
}

func (m *OutboxRepositoryMock) GetEvent(id uuid.UUID) *outbox.Event {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.events[id]
}
