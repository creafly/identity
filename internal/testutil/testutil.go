...
func SetupTestDB(t *testing.T) *TestDB {
	...
	if dsn == "" {
		dsn = "postgres://postgres:postgres@localhost:5440/postgres?sslmode=disable"
	}
...
func runMigrations(db *sqlx.DB) error {
	...
	if schemaResetErr != nil {
		return fmt.Errorf("failed to reset schema: %w", schemaResetErr)
	}
...
func NewTestOutboxEvent() *outbox.Event {
	return outbox.NewEvent("test.event", `{"test": "data"}`)
}
